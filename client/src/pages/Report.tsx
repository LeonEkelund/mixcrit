...
function Report() {
  ...
  return (
    <div className="min-h-screen px-4 pt-28 pb-12 max-w-5xl mx-auto flex flex-col items-center">
      ...
      {/* Waveform Player */}
      {file && (
        <div className="mb-8 w-full rounded-xl border border-border bg-background p-4 flex items-center gap-4">
          <button
            onClick={() => wavesurferRef.current?.playPause()}
            className="flex-shrink-0 flex items-center justify-center w-9 h-9 rounded-full bg-primary text-background hover:opacity-90 transition-opacity"
          >
            {isPlaying ? <Pause size={16} /> : <Play size={16} className="ml-0.5" />}
          </button>
          <div ref={waveformRef} className="flex-1 min-w-0" />
          <span className="flex-shrink-0 text-sm text-muted-foreground font-mono">
